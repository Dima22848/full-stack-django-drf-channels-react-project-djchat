...
class Channel(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.id:
            existing = Channel.objects.filter(id=self.id).first()  # <-- Исправлено получение существующего объекта
            if existing:
                if existing.banner != self.banner:
                    existing.banner.delete(save=False)
                if existing.icons != self.icons:
                    existing.icons.delete(save=False)
        super(Channel, self).save(*args, **kwargs)  # <-- Вызов super() для Channel

    @receiver(models.signals.pre_delete, sender=Server)  # <-- Исправлен sender
