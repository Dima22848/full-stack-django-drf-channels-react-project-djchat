...
class Server(models.Model):
    ...
    banner = models.ImageField(upload_to=server_banner_upload_path, null=True, blank=True, validators=[validate_icon_image_size, validate_image_file_exstension])
    icons = models.ImageField(upload_to=server_icon_upload_path, null=True, blank=True, validators=[validate_icon_image_size, validate_image_file_exstension])

    def save(self, *args, **kwargs):
        if self.id:
            existing = get_object_or_404(Server, id=self.id) # <-- Исправлено получение существующего объекта
            if existing:
                if existing.banner != self.banner:
                    existing.banner.delete(save=False)
                if existing.icons != self.icons:
                    existing.icons.delete(save=False)
        super(Server, self).save(*args, **kwargs)  # <-- Вызов super() для Channel

    @receiver(models.signals.pre_delete, sender='server.Server')  # <-- Исправлен sender
    def server_delete_files(sender, instance, **kwargs):
        for field in ['banner', 'icons']:
